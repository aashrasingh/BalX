...
-- these when SMTP is configured in .env.
INSERT INTO users (email, name, role) VALUES
  ('workprakriti11@gmail.com',    'Prakriti (Parent)', 'parent'),
  ('aashrasingh282@gmail.com',    'Aashra (Parent)',    'parent'),
  ('jaybirmalla@gmail.com',       'Jaybir (Parent)',    'parent'),
  ('swornim1029@gmail.com',       'Swornim (Parent)',   'parent'),
  ('garimadangol012@gmail.com',  'Garima (Parent)',    'parent'),
  ('studentservice444@gmail.com', 'Student Services',   'ssd_staff')
ON CONFLICT (email) DO NOTHING;

INSERT INTO students (student_id, name, grade, section) VALUES
  ('STU001', 'Aarav Karki',   'Grade 12', 'A'),
  ('STU002', 'Maya Karki',    'Grade 11', 'B'),
  ('STU003', 'Rohan Sharma',  'Grade 12', 'C'),
  ('STU004', 'Nisha Sharma',  'Grade 11', 'A'),
  ('STU005', 'Sijan Thapa',   'Grade 10', 'A'),
  ('STU006', 'Kavi Gurung',   'Grade 12', 'B'),
  ('STU007', 'Laxmi Gurung',  'Grade 11', 'C')
ON CONFLICT (student_id) DO NOTHING;

-- Each test parent linked to at least one student, so every account has
-- something real to see in the parent portal. Sijan (STU005) is the
-- deliberately at-risk student used throughout the automation demo.
-- Looked up BY EMAIL (not hardcoded IDs) so this works correctly even if
-- this table already had earlier rows in it from a previous version.
INSERT INTO parent_students (parent_id, student_id)
SELECT u.id, x.student_id FROM (VALUES
  ('workprakriti11@gmail.com',   'STU001'),
  ('workprakriti11@gmail.com',   'STU002'),
  ('aashrasingh282@gmail.com',   'STU003'),
...
-- Self-healing: add `cleared` if this table already existed from an
-- earlier version of the project, without it.
