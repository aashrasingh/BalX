...
export async function getStaffStats() {
  ...
  const [students, summaries, tickets, users, modules] = await Promise.all([
    query("SELECT student_id FROM students"),
    query("SELECT student_id, risk_level FROM attendance_summary"),
    query("SELECT status, cleared FROM tickets"),
  ...
  return { totalStudents, totalModules, totalTickets, openTickets, resolvedTickets, activeAlerts, byRisk, parentAccounts, staffAccounts };
}
