...
export async function getAllTickets({ status, priority, includeCleared = false } = {}) {
  ...
  const { rows } = await query(
    `SELECT t.*, s.name AS "studentName" FROM tickets t
...
export async function updateTicket(id, { status, assignedTo, cleared }) {
  if (isDemoMode) return demo.updateTicket(id, { status, assignedTo, cleared });

  if (cleared !== undefined) {
    const { rows: currentRows } = await query("SELECT status FROM tickets WHERE id = $1", [id]);
    const current = currentRows[0];
    if (!current) return null;
    if (cleared && current.status !== "resolved") {
  ...
  const { rows } = await query(
    ...
    [id, status || null, assignedTo || null, cleared === undefined ? null : cleared]
  );
  return rows[0] || null;
}
