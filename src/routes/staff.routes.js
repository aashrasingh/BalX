...
// Real dashboard numbers (counts of students, tickets, alerts, etc.) computed
// straight from the database — the staff UI must never hardcode these totals.
...
router.get("/activity", async (req, res) => {
  const data = await getStaffActivityFeed(10);
  res.json({ success: true, data });
});

router.get("/fee-leaderboard", async (req, res) => {
  const data = await getFeeLeaderboard();
  res.json({ success: true, data });
});
...
router.post("/appointments", async (req, res) => {
  const { studentId, parentName, type, apptDate, apptTime, notes } = req.body || {};
  if (!parentName || !type || !apptDate) {
    return res.status(400).json({ success: false, message: "parentName, type, and apptDate are required." });
  }
  const appt = await createAppointment({ studentId, parentName, type, apptDate, apptTime, notes, createdBy: req.user.id });
  res.status(201).json({ success: true, data: appt });
});

router.patch("/appointments/:id", async (req, res) => {
  const { status, apptDate, apptTime, notes } = req.body || {};
  const updated = await updateAppointment(req.params.id, { status, apptDate, apptTime, notes });
...
router.patch("/tickets/:id", async (req, res) => {
  const { status, assignedTo, cleared, reply } = req.body || {};
  const updated = await updateTicket(req.params.id, { status, assignedTo, cleared, reply });
