...
- Urgent notices — broadcast or targeted — with a real, persistent
  "mark as read" (dismissing one server-side means it won't reappear on
  your next login, not just hidden until refresh)
- "Message Student Services" ticketing, both sides (parent creates, staff
  manages status). Once a request is marked resolved, a "Clear" button
  appears for both the parent and staff — enforced server-side, so neither
  side can hide a request that isn't actually resolved yet
...
the code as such):** grades, appointments scheduling, lecturer directory,
notification settings, staff performance charts, staff activity feed, staff
directory.
...
- **Parent portal:** the attendance table, the clearance card, the fees
  table, and the "Attendance risk check" card are all real data for
  whichever child is selected. Notice the Tutorial/Workshop/Lecture badges
  are color-coded, and the "By subject" card above the table shows real
  present/late/absent counts per module. If a parent has two children (like
  workprakriti11), a dropdown appears in the top bar to switch between them.
  Submit a message under "Message Student Services" — it actually creates a
  ticket. Click "Mark as read" on an urgent notice, then reload the page —
  it stays dismissed, because it's saved server-side, not just hidden in
  the browser.
- **Staff portal:** type "Sijan" into the Attendance Log Lookup search box —
  ...
  red/amber cells. Click one to see the reason. Check the "Student alerts"
  panel — those are pulled directly from the automated risk engine, not
  hardcoded. Try composing an urgent notice targeted at `STU005` (Sijan) —
  it looks up his real linked parent (jaybirmalla@gmail.com) and actually
  emails them; watch the terminal for the `[notify:EMAIL] Sent to
  jaybirmalla@gmail.com` line to confirm it went out for real. That same
  real-email path now also fires automatically whenever the attendance sync
  flags a student's risk level getting worse — check the terminal right
  after server startup, you'll see a batch of `[notify:EMAIL...]` lines for
  every already-at-risk student. In the Student Requests table, mark a
  request "Resolved" — a clear (✓) icon appears next to it; it stays greyed
  out and unclickable on anything still open or in progress.
...
### Twilio (SMS) — incomplete, needs 2 more things from you
Only the Account SID was provided. Real SMS also needs:
1. **Auth Token** — Twilio Console → Account → API keys & tokens
2. **A Twilio phone number** — Console → Phone Numbers (trial accounts get
   one free number, but can only text numbers you've verified in the console)

Add both to `.env` (`TWILIO_AUTH_TOKEN`, `TWILIO_FROM_NUMBER`), then run
`npm install twilio`. Until then, SMS just logs to the console — which
still lets you demo the workflow, just without a real text arriving.
